...
const ListPayed = () => {
  ...
  const handleListMilitant = async() => {
    setLoading(true)
    const response = await getDemandesPayer(auth.token)
    if(response.success) {
  ...
  return (
    <>
      <PageMeta
        title="Bloc Républicain | Liste des demandes payées"
        description="Cette page affiche la liste des demandes payées."
      />
