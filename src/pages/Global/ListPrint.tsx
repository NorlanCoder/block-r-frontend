...
import { getDemandesImprimer, getDemandesNonImprimer } from "../../api/supAdmin";
...
const ListPrint = () => {
  ...
  const handleListMilitant = async() => {
    setLoading(true)
    const response = await getDemandesImprimer(auth.token)
    if(response.success) {
  ...
  return (
    <>
      <PageMeta
        title="Bloc Républicain | Liste des demandes imprimées"
        description="Cette page affiche la liste des demandes imprimées."
      />
